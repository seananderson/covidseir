...
proj <- covidseir::project_seir(fit1, iter = 1:25)
...
start_decline_fake <- c(log(0.5), 0.2)
end_decline_fake <- c(log(2), 0.1)
...
# FIXME: is state0 at time -30 or 0?
S0 <- proj1 %>% filter(time == max(time)) %>% 
  group_by(variable) %>% 
