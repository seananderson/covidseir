...
d$x_r[["fixed_f_forecast"]] <- 0.6
d$x_r

days_back <- 45L
forecast_days <- 120L
days <- seq(1, nrow(d$daily_cases) + forecast_days)
...
d$x_i[1] <- length(d$x_i) - 1
d$n_x_i <- length(d$x_i)

pars_save <- c("phi", "mu", "y_rep")

initf <- function(post, i, stan_data) {
  R0 <- post$R0[i]
...
out <- furrr::future_map_dfr(1:50, function(i) {
  fit <- rstan::sampling(
    covidseir:::stanmodels$seir,
    data = d,
    iter = 1L,
    chains = 1L,
    init = function() initf(p, i, d),
    pars = pars_save,
    refresh = 0,
  ...
  tibble(
    day = d$days,
    mu = post$mu[, , 1],
    phi = rep(post$phi[1], length(d$days)),
    y_rep = post$y_rep[, , 1],
    iterations = rep(i, length(d$days)) 
  )
})

ggplot(out, aes(day, mu, group = iterations)) + geom_line()
ggplot(out, aes(day, y_rep, group = iterations)) + geom_line(alpha = 0.1)
```
