...
dat <- tibble(
  ...
  date = rep(seq(lubridate::ymd("2020-03-01"), 
    lubridate::ymd("2020-03-01") + length(hosp) - 1, by=  "1 day"), 2))
...
```{r, include=FALSE}
# post_state <- m1$fit %>% tidybayes::spread_draws(y_hat[time_num, state_num], n = 20)
# variables_df <- tibble(
...
ggplot(post_mu, aes(day, lambda_d, group = paste(.iteration, .draw))) +
  ...
  geom_point(data = filter(dat, type == "daily_cases"), aes(x = day, y = cases),
    inherit.aes = FALSE)

ggplot(post_y_rep, aes(day, y_rep, group = paste(.iteration, .draw))) +
  geom_line(alpha = 0.07) +
  facet_wrap(~data_type) +
  geom_line(data = filter(dat, type == "daily_cases"), aes(x = day, y = cases),
    inherit.aes = FALSE)
```
...
proj$states %>% tidyr::pivot_longer(-c(time, iterations)) %>% 
...
```{r}
f_vec <- c(rep(NA, m1$last_day_obs),
...
```{r}
make_projection_plot(list("One model" = m1, "Another model" = m1))
```
...
```{r fit2types, message=FALSE, results='hide', cache=TRUE}
s1 <- c(rep(0.1, 13), rep(0.2, length(cases)-13)) # example sample fractions
s2 <- rep(0.05, length(hosp)) # example sample fractions
...
```{r}
make_projection_plot(list("Scenario 1" = m2))
```
