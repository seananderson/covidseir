...
m0 <- fit_seir(cbind(d$cases, hosp), iter = 50, chains = 1,
  sampled_fraction_vec = cbind(s1, s2), save_state_predictions = TRUE)
...
f_vec <- c(rep(NA, m$last_day_obs), c(rep(0.8, 7)))

p <- list()
p[[1]] <- project_fit(m, f_vec = f_vec, 
  proj_days = sum(!is.na(f_vec)), i = 1:10, data_col = 2L)
...
for (i in 2:5) {
  ...
  mean_lamb <- dplyr::filter(p[[i-1]]$cases, day == max(day)) %>% 
    pull(lambda_d) %>% mean()
  if (mean_lamb > 70) {
    f_vec <- c(f_vec, rep(0.18, 7))
  } else {
    f_vec <- c(f_vec, rep(0.8, 7))
  }
  p[[i]] <- project_fit(m, f_vec = f_vec, proj_days = sum(!is.na(f_vec)), i = 1:10)
}

library(ggplot2)
ggplot(p[[5]]$states, aes(time, I + Id, group = iterations)) + 
  geom_line(alpha = 0.3)

ggplot(p[[5]]$cases, aes(day, lambda_d, group = iterations)) + 
  geom_line(alpha = 0.3)
