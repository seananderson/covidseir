...
create_segments_vector <- function(start_date,total_days,segments,values=NULL){
  if(!is.null(values) & (length(values) != (length(segments) + 1))){
    stop("Length values must be length segments + 1.")
  }

  if(is.null(values)){
    values <- 1:(length(segments) + 1)
  }
...
#'              increasing to `ramp_max` between `start_ramp` and `end_ramp`.
#'              Useful for characterizing the change in transmission due to
#'              Variants of Concern.
#' @param start_date date at start of data (in ymd format)
#' @param total_days int Number of days in data or projection. Gives total
#'                   length of array
...
#' ramp_length <- 10
#' ramp_max <- 2
#' create_segments_vector(start_date,total_days,start_ramp,ramp_length,
#'                       ramp_max)
#' @importFrom lubridate ymd days
#' @export
create_ramp_vector <- function(start_date,total_days,start_ramp,
  ...
  vec[s1:s2] = 1 + (ramp_max - 1)*(0:(s2 - s1))/ramp_length;

  # only include if total_days greathr than end of ramp
  if(total_days > s2){
    vec[(s2+1):total_days] = ramp_max
  }
